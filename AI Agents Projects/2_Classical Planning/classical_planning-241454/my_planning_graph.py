...
class LiteralLayer(BaseLiteralLayer):
    ...
    def _inconsistent_support(self, literalA, literalB):
        ...
        #Create multiple combinations of actions
        ...
        #Check multiple combinations of actions
...
class PlanningGraph:
    ...
    def h_levelsum(self):
        ...
        while len(goals_tracked) < len(self.goal):
            layer_checked = self.literal_layers[-1]
            for literal in layer_checked:
                if literal in self.goal and literal not in goals_tracked:
                    goals_tracked.append(literal)
    ...
    def h_setlevel(self):
        ...
        while True:
            layer_checked = self.literal_layers[-1]
            if set(self.goal).issubset(layer_checked):
                is_mutex = False
                for goalA in self.goal:
                    for goalB in self.goal:
                        if layer_checked.is_mutex(goalA, goalB):
                            is_mutex = True
                            break
                    if is_mutex:
                        break
