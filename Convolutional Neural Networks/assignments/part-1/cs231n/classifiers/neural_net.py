...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    num_train = X.shape[0]
    exp_val = np.exp(X2)
    #scores  = exp_val/np.sum(exp_val, axis = 1)
    ...
    #W2 and b2
    grads['W2'] = np.dot(X1.T,probs)
    grads['b2'] = np.sum(probs, axis=0)
    grads['W2'] += 2 * reg * W2
    
    #backpropgation into hidden layer
    dX1 = np.dot(probs,W2.T)
    dX1[X1<=0] = 0
    
    #W1 and b1
    grads['W1'] = np.dot(X.T, dX1)
  ...
  def predict(self, X):
    ...
    X1 = np.maximum(0,X.dot(self.params['W1'])+self.params['b1'])
